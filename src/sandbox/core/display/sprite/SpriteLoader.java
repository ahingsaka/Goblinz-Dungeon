 ...
 * <p>
 * To use, call {@link #getSprite(String imageUrl, String jsonUrl)} with an
 * image path and json data, or {@link #getSprite(String jsonUrl)} with json
...
public class SpriteLoader {
    ...
    /**
     * Return a {@link Sprite}, given a path to the image and a path to the json
     * sprite description.
     * <p>
    ...
    public static Sprite getSprite(String imagePath, String jsonPath) {
        ...
        // temp image to prevent NPE if using the Sprite's Layer
        // (Sprite.getLayer()) before the image
    ...
    public static Sprite getSprite(String jsonPath) {
        ...
        // load and parse json, then add each image parsed from the json to the
        // asset watcher to load
        assetManager().getText(jsonPath, new ResourceCallback<String>() {
            @Override
            public void done(String json) {
                try {
     ...
     * 
     * @param image
     *            Image to associate with each {@link SpriteImage}, or null to
     *            parse from the json
     * @param sheet
     *            Sprite to store the {@link SpritesImage}s
     * @param json
     *            json to parse
     */
    private static void parseJson(Image[] images, Sprite sprite, String json) {
        ...
        for (int i = 0; i < spriteImages.length(); i++) {
            ...
            int imageId = jsonSpriteImage.getInt("url"); // will return 0 if not
                                                         // specified
